...
while current_row <= input_ws.max_row:
    check_1_cell = str(input_ws.cell(row=current_row, column=1).value)
    check_2_cell = str(input_ws.cell(row=current_row, column=2).value)
    check_3_cell = input_ws.cell(row=current_row, column=3).value

    if check_1_cell and check_2_cell and check_3_cell:
        if check_3_cell == "谐音":
            similarity_percentage = similar(check_1_cell, check_2_cell) * 100
            if len(check_2_cell) != len(check_1_cell) or similarity_percentage < 80:
