...
- **《{work}》系列**：{work_info},讲述了{plots}，{character}由({author}饰),{plots}。

- **《{work}》系列**：{work_info},讲述了{plots}，{character}由({author}饰),{plots}。

- **《{work}》系列**：{work_info},讲述了{plots}，{character}由({author}饰),{plots}。

    用户请求：{action?还有其他的的吗:还有吗:其他的还有吗:除了这些还有吗}

    标注回答：除了《{work}》《{work}》《{work}》{action?{author}还有什么代表作:还演过什么}
...
if __name__ == '__main__':
    ...
    while True:
        ...
        print(query)
        result = gpt4o.gpt_4o_request(query)
        print(result)
